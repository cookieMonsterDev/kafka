...
Integration tests historically cover **0.10, 0.11, 1.1, 2.4, 3.6, and 4.0**.
Default `KAFKA_VERSION` remains **4.0**. Compose files exist for 4.1, 4.2, and
4.3 (`apache/kafka:4.1.2` / `4.2.1` / `4.3.1`); CI PRs run 4.3 and default-branch
pushes run the full matrix.

The client talks to **4.0 today** via overlap (Produce 3–7, Fetch 4–11). Kafka
...
| ------------------------------ | ----------------------------------------- |
| 0.10, 0.11, 1.1, 2.4, 3.6, 4.0 | Historically covered by integration tests |
| 4.1, 4.2, 4.3                  | Compose files in tree; CI PRs run 4.3     |
