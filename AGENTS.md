...
- **`protocol/`** — wire format. `protocol/requests/<api-name>/v<N>/{request,response}.ts` encode/decode one Kafka API at one version; `protocol/message`, `message-set`, `records` handle the record batch formats (legacy MessageSet v0/v1 and modern RecordBatch v2); `protocol/compression` holds codecs (GZIP/Snappy/ZSTD built-in, LZ4 pluggable via `CompressionCodecs`); `protocol/sasl` handles SASL mechanism framing.
