...
import { advertisedAddress, createConnectionPool, newLogger, retryProtocol, secureRandom } from '../helpers/index.js';
  ...
  it('joins, heartbeats, syncs, and leaves a group', async () => {
    const join = await retryProtocol(['NOT_COORDINATOR_FOR_GROUP', 'GROUP_LOAD_IN_PROGRESS'], () =>
      coordinator!.joinGroup({
        ...
        groupProtocols: [{ name: 'AssignerName', metadata: MemberMetadata.encode({ version: 1, topics: [topicName] }) }],
      }),
    );
