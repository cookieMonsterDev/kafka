...
import { createDelegationTokenResponseV2 } from './response';
...
describe('protocol/requests/create-delegation-token/v2/response', () => {
  it('decodes a flexible body, remapping throttleTime', async () => {
    await expect(createDelegationTokenResponseV2.decode(responseFixture())).resolves.toEqual({
      errorCode: 0,
      principalType: 'User',
      principalName: 'alice',
  ...
  it('rejects a top-level protocol error', async () => {
    const data = await createDelegationTokenResponseV2.decode(responseFixture(61));
    await expect(createDelegationTokenResponseV2.parse(data)).rejects.toMatchObject({
