...
import { advertisedAddress, createConnectionPool, newLogger, retryProtocol, secureRandom, TRANSIENT_METADATA_ERRORS } from '../helpers/index.js';
...
describe('broker.adminApis', () => {
  ...
  it('creates and deletes topics through the controller', async () => {
    ...
    const created = await controller.createTopics({ topics: [{ topic: topicName, numPartitions: 1, replicationFactor: 1 }] });
