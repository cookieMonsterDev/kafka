...
describe('producer.instrumentation', () => {
  ...
  it('emits connect, request, and disconnect events', async () => {
    ...
    const events: string[] = [];
    producer.on(producer.events.CONNECT, () => events.push('connect'));
    producer.on(producer.events.DISCONNECT, () => events.push('disconnect'));
    producer.on(producer.events.REQUEST, () => events.push('request'));
