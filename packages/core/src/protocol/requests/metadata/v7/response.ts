 ...
 *
 * KIP-320 adds `leader_epoch` after `leader`. `offline_replicas` remains from v5 (KIP-112).
...
export type MetadataResponseV7Body = Omit<RawBody, 'throttleTime'> & {
  throttleTime: number;
  clientSideThrottleTime: number;
};

export const metadataResponseV7: ResponseDefinition<MetadataResponseV7Body> = {
