...
import { shareGroupHeartbeatResponseV1 } from './response';
...
describe('protocol/requests/share-group-heartbeat/v1/response', () => {
  it('decodes a null assignment struct', async () => {
    const data = await shareGroupHeartbeatResponseV1.decode(
    ...
    await expect(shareGroupHeartbeatResponseV1.parse(data)).resolves.toBe(data);
  });
  ...
  it('decodes an assignment with topic ids and partitions', async () => {
    ...
    await expect(shareGroupHeartbeatResponseV1.decode(responseFixture({ assignment }))).resolves.toEqual({
      throttleTime: 0,
      clientSideThrottleTime: 12,
      errorCode: 0,
      errorMessage: null,
  ...
  it('rejects a top-level protocol error', async () => {
    ...
    await expect(shareGroupHeartbeatResponseV1.parse(data)).rejects.toMatchObject({
