...
describe('broker/sasl-authenticator/scram512AuthenticatorProvider', () => {
  it('throws for a missing username', async () => {
    const scram = scram512AuthenticatorProvider({ username: undefined, password: 'pencil' })({
      host: '',
      port: 0,
      logger: silentLogger,
  ...
  it('throws for a missing password', async () => {
    const scram = scram512AuthenticatorProvider({ username: 'user', password: undefined })({
      host: '',
      port: 0,
      logger: silentLogger,
      saslAuthenticate: vi.fn(),
  ...
  it('sends a well-formed client-first-message and rejects a mismatched server nonce', async () => {
    ...
    const scram = scram512AuthenticatorProvider({ username: 'user', password: 'pencil' })({
      host: 'host',
      port: 9094,
      logger: silentLogger,
      saslAuthenticate,
