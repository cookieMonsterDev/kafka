...
import { mergeKafkaConfigLayers } from './merge';
...
describe('mergeKafkaConfigLayers', () => {
  describe('precedence matrix', () => {
    it.each([
      ...
      expect(mergeKafkaConfigLayers<Fixture>(override, base).clientId).toBe(expected);
    });

    it('omits a key defined nowhere, so the caller default still fires', () => {
      const result = mergeKafkaConfigLayers<Fixture>({}, {});

      expect(Object.hasOwn(result, 'clientId')).toBe(false);
    });

    it('omits a key that is undefined in both layers', () => {
      const result = mergeKafkaConfigLayers<Fixture>({ clientId: undefined }, { clientId: undefined });

      expect(Object.hasOwn(result, 'clientId')).toBe(false);
    });

    it('an explicit undefined in override does not clobber a defined base value', () => {
      const result = mergeKafkaConfigLayers<Fixture>({ clientId: undefined, untouched: 'x' }, { clientId: 'base' });
  ...
  describe('falsy-but-meaningful values survive', () => {
    it.each([
      ...
      const result = mergeKafkaConfigLayers<Fixture>(override, { [key]: 'should-be-overridden' });
    ...
    it('a falsy base value survives when override omits the key', () => {
      const result = mergeKafkaConfigLayers<Fixture>({}, { logLevel: 0, clientId: '' });
  ...
  describe('retry: shallow-merged one level', () => {
    ...
    it('takes retry from base alone when override never mentions it', () => {
      const result = mergeKafkaConfigLayers<Fixture>({}, { retry: { retries: 5 } });

      expect(result.retry).toEqual({ retries: 5 });
    });

    it('omits retry when neither layer defines any sub-key', () => {
      const result = mergeKafkaConfigLayers<Fixture>({ retry: {} }, {});
  ...
  describe('atomic replace for discriminated / holder values', () => {
    ...
    it('replaces ssl: true over ssl: {ca}', () => {
      const result = mergeKafkaConfigLayers<Fixture>({ ssl: true }, { ssl: { ca: 'base-ca' } });

      expect(result.ssl).toBe(true);
    });

    it('replaces ssl: {ca} over ssl: true', () => {
      const result = mergeKafkaConfigLayers<Fixture>({ ssl: { ca: 'override-ca' } }, { ssl: true });

      expect(result.ssl).toEqual({ ca: 'override-ca' });
    });

    it('does not concatenate brokers arrays', () => {
      const result = mergeKafkaConfigLayers<Fixture>({ brokers: ['override:9092'] }, { brokers: ['base:9092'] });
    ...
    it('a BrokersFunction replaces an array wholesale', () => {
      const fn = () => ['fn:9092'];
      const result = mergeKafkaConfigLayers<Fixture>({ brokers: fn }, { brokers: ['base:9092'] });
    ...
    it.each(['metrics', 'socketFactory', 'logCreator'] as const)('replaces %s atomically', (key) => {
      ...
      const result = mergeKafkaConfigLayers<Fixture>({ [key]: overrideValue }, { [key]: baseValue });
